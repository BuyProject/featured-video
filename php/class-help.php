...
class FVP_Help
{
	...
	public function post()
	{
		...
		$content[] = FVP_HTML::html(
			...
			esc_html__('Fixing upload errors', 'featuredvideo') . ':'
		);
	...
	public function functions()
	{
		...
		$content[] = FVP_HTML::html('p', sprintf(
			...
			'<code>$size</code>',
			'(<code>thumbnail</code>, <code>medium</code>, <code>large</code> ' . esc_html__('or', 'featuredvideo') . ' <code>full</code>)'
		));
	...
	public function shortcode()
	{
		...
		$content[] = FVP_HTML::unordered_list(array(
			'<code>[featuredvideo]</code><br />' .
				'<span>' .
				esc_html__('Displays the video in its default size.', 'featuredvideo') .
				'</span>',
			'<code>[featuredvideo width=560]</code><br />' .
				'<span>' .
				esc_html__('Displays the video with a width of 300 pixel. Height will be fitted such that the aspect ratio is preserved.', 'featuredvideo') .
				'</span>',
			'<code>[featuredvideo width=560 height=315]</code><br />' .
				'<span>' .
				esc_html__('Displays the video with a fixed width and height.', 'featuredvideo') .
				'</span>',
		));
